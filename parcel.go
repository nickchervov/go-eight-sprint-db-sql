...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("error when adding an entry %v", err)
	}
	// верните идентификатор последней добавленной записи
	lastId, err := res.LastInsertId()
	if err != nil {
		return 0, fmt.Errorf("error in calculating the ID of the last record: %v", err)
	}
...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	if err := row.Scan(&p.Number, &p.Client, &p.Status, &p.Address, &p.CreatedAt); err != nil {
		return Parcel{}, fmt.Errorf("error when filling the object with data from the request: %v", err)
	}
	return p, nil
}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	if err != nil {
		return []Parcel{}, fmt.Errorf("error when getting a rows by client from the db: %v", err)
	}
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	if err != nil {
		return fmt.Errorf("error updating the status in the table: %v", err)
	}
	return nil
}
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if err != nil {
		return fmt.Errorf("error updating the address in the table: %v", err)
	}
	return nil
}
...
func (s ParcelStore) Delete(number int) error {
	...
	if err != nil {
		return fmt.Errorf("error when deleting a record from the table: %v", err)
	}
	return nil
}
